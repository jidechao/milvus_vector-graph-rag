...
class VectorGraphRAG:
    ...
    def add_documents(
        ...
        2. Builds the knowledge graph structure for this batch only
        3. Appends entities, relations, and passages to existing Milvus collections
        ...
        # Append to existing Milvus data (do not drop collections).
        # Each batch inserts new rows; IDs are UUIDs so collisions are unlikely.
        self._store.create_collections(drop_existing=False)
    ...
    def query_stream(
        ...
        """
        Stream query progress and answer chunks.

        Yields event dicts with shape:
        - {"type": "status", "stage": "...", ...}
        - {"type": "answer_chunk", "delta": "..."}
        - {"type": "done", ...}
        """
        retriever = self._ensure_retriever()
        ...
        yield {
            ...
            "stage": "retrieved",
            "query_entities": retrieval_result.query_entities,
            "retrieved_relations": len(retrieval_result.relation_ids),
            "expanded_relations": len(candidate_ids),
        }

        if use_reranking and candidate_ids:
            yield {"type": "status", "stage": "reranking"}
            reranked_ids, _ = self._reranker.rerank(question, candidate_ids, candidate_texts)
        else:
            reranked_ids = candidate_ids[: self.settings.final_top_k]
        ...
        yield {
            ...
            "passage_ids": passage_ids[: self.settings.final_top_k],
        }

        full_answer_chunks: List[str] = []
        for delta in self._answer_generator.generate_stream(question, final_passages):
            full_answer_chunks.append(delta)
            yield {"type": "answer_chunk", "delta": delta}
