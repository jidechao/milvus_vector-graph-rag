...
def _should_use_openai_backend(model_name: str, settings: Settings) -> bool:
    """Decide whether to use OpenAI-compatible embedding backend."""
    # Explicitly configured OpenAI-compatible endpoint should take precedence.
    if settings.openai_base_url:
        return True
    return _is_openai_model(model_name)
...
class OpenAIEmbedding:
    ...
    def __init__(
        ...
        self._dimensions_supported = True
        ...
        def _call_api(texts):
            ...
            if self.dimensions and self._dimensions_supported:
                request_kwargs["dimensions"] = self.dimensions

            try:
                return self.client.embeddings.create(**request_kwargs)
            except Exception as e:
                # Some OpenAI-compatible providers don't implement `dimensions`.
                # Fall back once to keep compatibility.
                if "dimensions" in str(e).lower() and self._dimensions_supported:
                    self._dimensions_supported = False
...
class EmbeddingModel:
    ...
    def __init__(
        ...
        if _should_use_openai_backend(self.model_name, self.settings):
