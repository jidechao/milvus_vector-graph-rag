...
class MilvusStore:
    ...
    def _create_collection(
        ...
        # Dimension priority:
        # 1) Explicit create_collections(dimension=...)
        # 2) Configured VGRAG_EMBEDDING_DIMENSION
        requested_dim = dimension or self.settings.embedding_dimension
        detected_dim = self.embedding_model.dimension

        # Always align collection schema with actual embedding output dimension.
        # Otherwise inserts fail later with cryptic Milvus "length should divide dim" errors.
        if requested_dim and requested_dim != detected_dim:
            logger.warning(
                "Embedding dimension mismatch detected: requested=%s, model_output=%s. "
                "Using model_output dimension to create collection '%s'.",
                requested_dim,
                detected_dim,
                collection_name,
            )
    ...
    def _insert_data(
        ...
        # Validate embedding shapes early for clearer errors.
        if embeddings:
            expected_dim = len(embeddings[0])
            invalid_idx = next(
                (i for i, emb in enumerate(embeddings) if len(emb) != expected_dim),
                None,
            )
            if invalid_idx is not None:
                raise ValueError(
                    f"Inconsistent embedding dimensions in batch: index 0 has dim {expected_dim}, "
                    f"but index {invalid_idx} has dim {len(embeddings[invalid_idx])}."
                )
    ...
    def list_graphs(
        ...
        for collection in all_collections:
            # Handle default graph without prefix:
            # vgrag_entities / vgrag_relations / vgrag_passages
            if collection == entity_suffix:
                relation_col = relation_suffix
                passage_col = passage_suffix
                has_relations = relation_col in all_collections
                has_passages = passage_col in all_collections

                graphs.append(
                    {
                        # Distinct from prefix "default" (default_vgrag_*). API maps this to no prefix.
                        "name": "unprefixed",
    ...
    def delete_graph(
        ...
        # API graph name "unprefixed" = bare vgrag_* (no collection_prefix)
        if graph_name in ("unprefixed", "__unprefixed__"):
            entity_col = entity_suffix
            relation_col = relation_suffix
            passage_col = passage_suffix
        else:
    ...
    @staticmethod
    def _row_count_from_stats_dict(raw: Optional[Dict[str, Any]]) -> int:
        """Parse row count from get_collection_stats() (key names vary by Milvus / Zilliz build)."""
        if not raw:
            return 0
    ...
    def _count_star_query(self, collection_name: str, expr: str, consistency_level: str) -> Optional[int]:
        """Run count(*) aggregation; returns None if query fails or yields no parseable count."""
        try:
            ...
            logger.debug(
                "count(*) query failed for %s expr=%r consistency=%s: %s",
                collection_name,
                expr,
                consistency_level,
                e,
    ...
    def _accurate_row_count(self, collection_name: str) -> int:
        """
        Return row count for UI stats, aligned with query-visible rows when possible.

        ``get_collection_stats`` / segment metadata often **under-counts** (e.g. growing
        segments). Do **not** return early on a positive ``row_count``; prefer ``count(*)``
        after ``load_collection``, trying ``Strong`` consistency first per Milvus guidance.
        """
        ...
        stats_n = 0
        try:
            raw = self.client.get_collection_stats(collection_name)
            stats_n = self._row_count_from_stats_dict(raw)
        except Exception as e:
            logger.debug("get_collection_stats failed for %s: %s", collection_name, e)

        try:
            self.client.load_collection(collection_name=collection_name, timeout=120)
        except Exception as e:
            logger.debug("load_collection for stats skipped/failed for %s: %s", collection_name, e)

        configured = getattr(self.settings, "milvus_consistency_level", None) or "Bounded"
        # Strong first: Milvus docs recommend it for authoritative count(*)
        consistency_order = ["Strong", configured, "Bounded", "Eventually", "Session"]
        consistency_levels: List[str] = []
        for c in consistency_order:
            if c not in consistency_levels:
                consistency_levels.append(c)

        for expr in ("", 'id != ""'):
            for cl in consistency_levels:
                qn = self._count_star_query(collection_name, expr, cl)
                if qn is not None:
                    return qn
    ...
    def get_collection_stats(self) -> Dict[str, int]:
        ...
        if self.client.has_collection(self.entity_collection):
            stats["entity_count"] = self._accurate_row_count(self.entity_collection)

        if self.client.has_collection(self.relation_collection):
            stats["relation_count"] = self._accurate_row_count(self.relation_collection)

        if self.client.has_collection(self.passage_collection):
            stats["passage_count"] = self._accurate_row_count(self.passage_collection)
